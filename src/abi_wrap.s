.text
.globl _call_sum_abi_checked
.p2align 2
/* uint64_t call_sum_abi_checked(const uint64_t *a, size_t n, int *abi_ok);
 * Sets callee-saved sentinels, calls sum_array, reports ABI corruption via *abi_ok. */
_call_sum_abi_checked:
    /* Keep 16-byte alignment; reserve slot for abi_ok* (x2 must survive the call). */
    stp x29, x30, [sp, #-112]!
    stp x19, x20, [sp, #16]
    stp x21, x22, [sp, #32]
    stp x23, x24, [sp, #48]
